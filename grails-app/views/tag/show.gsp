	...
	<body>
		...
		<div id="show-tag" class="content scaffold-show" role="main">
			...
			<ol class="property-list tag">
				...
				<g:if test="${tagInstance?.eaterys}">
				<li class="fieldcontain">
					<span id="eaterys-label" class="property-label"><g:message code="tag.eaterys.label" default="Eaterys" /></span>
					
						<g:each in="${tagInstance.eaterys}" var="e">
						<span class="property-value" aria-labelledby="eaterys-label"><g:link controller="eatery" action="show" id="${e.id}">${e?.encodeAsHTML()}</g:link></span>
						</g:each>
